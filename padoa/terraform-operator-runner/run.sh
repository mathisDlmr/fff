...
function run_terraform {
    ...
    terraform plan $TFOPS_VARFILE_FLAG $destroy_when_true -lock=false -out plan.out 2>&1| tee $TMP
    ...
    terraform plan $TFOPS_VARFILE_FLAG $destroy_when_true -lock=false 2>&1| tee $TMP
}
